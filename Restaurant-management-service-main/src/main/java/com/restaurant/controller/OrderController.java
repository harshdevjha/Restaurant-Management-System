...
public class OrderController {
    ...
    private static void updateTableStatus(int tableId, RestaurantTable.Status status) throws SQLException {
    ...
    public static boolean updateOrderItemQuantity(int orderItemId, int newQuantity) {
        if (newQuantity <= 0) return removeOrderItem(orderItemId);
    ...
    /**
     * Fetches the active (PENDING or SERVED) order for a table, including all items.
    ...
    public static boolean markOrderBilled(int orderId, int tableId) {
        ...
        try (Connection conn = DBConnection.getConnection();
            ...
            if (ok) updateTableStatus(tableId, RestaurantTable.Status.FREE);
            return ok;
        } catch (SQLException e) {
